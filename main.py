...
im = util.resize_image(im, 5)
original_size = im.size
im = im.rotate(image_rotation, expand=True)
a = np.asarray(im)

intervals = (interval.threshold_interval(a, np.shape(a), 0.6, 0.75, func=util.lightness, inverted=True))

intervals = interval.randomly_filter_interval(intervals, 0.3)
pdb.set_trace()
colors = [np.array([255, 0, 0]), np.array([0, 255, 0]), np.array([0, 0, 255]), np.array([255, 255, 0]), np.array([255, 0, 255]), np.array([0, 255, 255])]

ac = a.copy()

new = sort_pixels(ac, intervals, sorting_func=util.saturation, reverse=False)
